...
async function listarUsuarios() {
  ...
  if (!response.ok) {
    ...
    for (const item of data) {
      ...
      if (item.status == true) {
        status.innerHTML = `<button class="btnadm" onclick='alterarStatus(${item.id})'>Ativo</button>`
      } else {
        status.innerHTML = `<button class="btnadm" onclick='alterarStatus(${item.id})'>Bloqueado</button>`
      }

      editar.innerHTML = `<button class="btneditar" onclick='editaUsuario(${item.id})'>Editar</button>`
...
async function alterarStatus(id) {
  const comentario = window.prompt("Deixe um comentário sobre esse chamado")
  const apiUrl = 'http://127.0.0.1/status/' + id
  console.log(apiUrl)
  const response = await fetch(apiUrl, {
    method: 'POST', // Corrigindo o método da requisição
    headers: {
      'Content-Type': 'application/json' // Especificando o tipo de conteúdo
    },
    body: JSON.stringify({ comentario: comentario}) // Convertendo o objeto JSON em uma string JSON
  })
...
async function verificarUsuario() {
  try {
    ...
    if (!response.ok) {
      ...
      if (ativo) {
        resultado.textContent = "Olá " + usuario + "!"
      }
      else {
        resultado.textContent = usuario + "Nenhum cadastro foi registrado nesse CPF."
      }
...
async function editarUsuario(cpf) {
  try {
    ...
    if (!response.ok) {
      alert('Chamado não encontrado (JS 166)!')
    }
    else {
      const data = await response.json()
